...
class BookControllers
{
    ...
    const BOOK_SALEOFF_STATE = 1;
    const BOOK_UNVERIFY_STATE = 2;
    const BOOK_VERIFIED_STATE = 3;
    ...
    function __construct() {
        ...
        $this->bookfields = BookFields::instance();
    ...
    */
    ...
    public function getBooksRow($option = array())
    {
        ...
        $list = $this->book->field("$table.bid,$table.cid,bc.name,$table.title,$table.author,i.path as cover,$table.pubtime,$table.isbn,$table.press,f.apple_price as price,$table.summary,f.tags,f.copyright,f.download_path as path,f.designer,f.proofreader,f.wordcount,f.dateline,bf.uid,bf.verified,bf.published")
    ...
    public function getBooksList($option = array(),$limit = 10,$page = 1)
    {
        ...
        $list = $this->book->field("$table.bid,$table.cid,$table.title,$table.author,bc.name as category,i.path as cover,$table.pubtime,$table.isbn,$table.press,f.apple_price as price,$table.summary,f.tags,bi.price,bi.apple_price,bf.verified,bf.published,m.username")
            ->joinQuery("book_info as f","$table.bid=f.bid")
            ->joinQuery('book_image as p',"$table.bid=p.bid")
            ->joinQuery('images as i','i.pid=p.pid')
            ->joinQuery('book_fields as bf',"$table.bid=bf.bid")
    ...
    /**
     * [changedBookStatus description]
     * @param  [type] $status [description]
     * @return [String]       [description]
     */
    public function changedBookStatus($status = 1)
    {
        $state = '等待审核';
        switch ($status) {
            case self::BOOK_WATTING_PUBLISH_STATE:
                $state = "等待审核";
                break;
            case self::BOOK_PUBLISHING_STATE:
                $state = "审核中";
                break;
            case self::BOOK_UNPUBLISHED_STATE:
                $state = "审核失败";
                break;
            case self::BOOK_PUBLISHED_STATE:
                $state = "审核通过";
                break;
            default:
                $state = "等待审核";
    ...
    /**
     * [changedBookVerified description]
     * @param  integer $status [description]
     * @return [type]          [description]
     */
    public function changedBookVerified($status = 1)
    {
        $state = '未发布';
        switch ($status) {
            case self::BOOK_SALEOFF_STATE:
                $state = "已下架";
                break;
            case self::BOOK_UNVERIFY_STATE:
                $state = "未发布";
                break;
            case self::BOOK_VERIFIED_STATE:
                $state = "已发布";
                break;
            default:
                $state = "已下架";
