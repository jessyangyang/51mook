...
class ORM extends MySQL
{
    ...
    */
    private function _options($option = array()) {
        ...
        if ($math = array_intersect(array_keys($tmpOption),$this->allowMathMethods)) {
           $tmpOption['field'] = $math[0];
        }
        else empty($tmpOption['field']) and $tmpOption['field'] = '*';
    ...
    private function _filter(&$dbData){
        ...
        foreach ($dbData as $key => $value){
            ...
            foreach (array('\\'=> '', "'"=>'`') as $key_2 => $var2)
            {
                ...
                {
                    $value = str_replace($key_2, $this->escapeString($var2), $value);
                }
    ...
    public function update($dbData) {
        if ($this->_filter($dbData)) {
            ...
            if ($dbData and isset($tmpOption['where']) and $tmpOption['where'])
            {
                ...
                self::$db->query($sql);
                return self::$db->affected_rows() ? self::$db->affected_rows() : false;
            }
