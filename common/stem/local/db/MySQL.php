...
class MySQL
{
    ...
    */
    static function init($primary_key = 0,$dbConfigName = false) 
    {
        ...
        if ($socket) {
            try {
                self::$dbo = new \mysqli($param['host'],$param['user'],$param['password'],$param['schema']);
    ...
    *******************************************/
    ...
    function selectTable($tableName) {
        return self::$dbo->select_db($tableName);
    }
    ...
    {
        return self::$dbo->set_charset($charset);
    }
     ...
     */
    function getTables() {
        return self::$dbo->query("SHOW TABLES")->fetch_all(3);
    }
    ...
    function getFields($dbTable) {
        return self::$dbo->query("SHOW FULL FIELDS FROM ".$dbTable)->fetch_all(2);
    }
    ...
    */
    ...
    {
        return self::$dbo->real_escape_string($fields);
    }
    ...
    */
    static function printDebug($message = '' , $sql = '') {
        $dberror = self::$dbo->error;
        $dberrno = self::$dbo->errno;
        $debug = 1;
